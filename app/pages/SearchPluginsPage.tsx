...
const SearchPlugin = ({ plugin }: { plugin: TorrPlugin }) => {
	...
	const { mutate: toggleEnable } = useMutation("enablePluginToggle", () => TorrClient.togglePluginEnabled(plugin.name, !plugin.enabled), {
		onSuccess: () => {
			setTimeout(async () => {
	...
	const { mutate: uninstall } = useMutation("uninstallPlugin", () => TorrClient.uninstallPlugin(plugin.name), {
		onSuccess: () => {
			setTimeout(async () => {
				await queryClient.invalidateQueries(SearchPluginsPageQuery);
			}, 500);
	...
	const backgroundColor = useColorModeValue("white", isLarge ? "black" : "gray.900");
...
const PublicPlugin = ({
	...
	const { mutate: addPlugin, isLoading: addLoading } = useMutation((path: string) => TorrClient.installPlugin(path), {
		onSuccess: (res, path) => {
			onSuccess(path);
...
const SearchPluginsPage = () => {
	const { data, refetch } = useQuery(SearchPluginsPageQuery, TorrClient.getInstalledPlugins);

	const [pluginLocation, setPluginLocation] = useState("");
	const [added, setAdded] = useState<string[]>([]);

	const { data: publicPlugins } = useQuery("getPublicPlugins", async () => {
		const { data } = await axios.get<TorrPublicPlugin[]>("https://iqbit.app/api/plugins");
		return data;
	});

	const { mutate: addPlugin, isLoading: addLoading } = useMutation(() => TorrClient.installPlugin(pluginLocation), {
		onSuccess: () => {
	...
	return (
		<>
			...
			<IosBottomSheet title={"qBitTorrent Public Plugins"} disclosure={publicPluginsDisclosure} modalProps={{ size: "2xl" }}>
				<Text my={3}>
