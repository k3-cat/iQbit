...
const SearchPage = () => {
	...
	const [selectedProvider, setSelectedProvider] = useState<ProviderKeys>((location?.state as any)?.provider || "YTS");
	const [selectedCategory, setSelectedCategory] = useState(0);

	const searchState = useState(((location?.state as any)?.query as string) || "");

	const { data: plugins, isLoading: pluginsLoading } = useQuery(SearchPluginsPageQuery, TorrClient.getInstalledPlugins);
