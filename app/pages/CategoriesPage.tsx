...
const CategoriesPage = () => {
	const { data, refetch } = useQuery("getCategoriesPage", TorrClient.getCategories);

	const location = useLocation();
	const isPage = location.pathname === Pages.find((page) => page.label === "Categories")?.url;
	...
	const { mutate: saveCategory, isLoading: saveCategoryLoading } = useMutation(
		...
		async () => {
			if (selectedCategory.mode === "Add") {
				await TorrClient.addCategory(selectedCategory.cat.name, selectedCategory.cat.savePath);
			} else {
				await TorrClient.saveCategory(selectedCategory.cat.name, selectedCategory.cat.savePath);
			}
		},
		{
			onSuccess: async () => {
	...
	const { mutate: removeCategory, isLoading: removeCategoryLoading } = useMutation("removeCategory", TorrClient.removeCategories, {
		onSuccess: async () => {
			await refetch();
			editDisclosure.onClose();
		},
		...
		<>
			{isLarge || isPage ? (
				...
				<MobileSettingsAddButton
					...
					onClick={() => setSelectedCategory({ mode: "Add", cat: addCategoryObject })}
				/>
			)}
