...
const TrendingPage = () => {
	...
	const { data: plugins } = useQuery(SearchPluginsPageQuery, TorrClient.getInstalledPlugins);
	...
	const providerMapper = useMemo(() => {
		return Object.entries(providers)
			.filter(([key]) => key !== "YTS")
			.filter(([key]) => ((plugins?.length || 0) > 0 ? true : key !== "plugin"));
	}, [plugins?.length]);
		...
		<>
			...
			{tab === 0 ? (
				<PosterGrid
					...
					onSelect={(movie) => {
						getTorrs(`${movie?.title} ${movie?.release_date?.split("-")?.[0] || ""}`);
											...
											<Button
												...
												onClick={() =>
													push("/search", {
														...
														state: {
															provider: key,
															query: `${selectedMovie?.title} ${selectedMovie?.release_date?.split("-")?.[0]}`,
														},
													})
												}
											>
