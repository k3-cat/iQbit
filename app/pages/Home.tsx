...
const Home = () => {
	...
	const { data: categories } = useQuery("torrentsCategory", TorrClient.getCategories);
	...
	const { data: settings } = useQuery("settings-mainpage", TorrClient.getSettings, { refetchInterval: 30000 });
	...
	const { mutate: attemptAddTorrent, isLoading: attemptAddLoading } = useMutation(
		"addTorrent",
		(opts: { autoTmm?: boolean }) => TorrClient.addTorrent(!!textArea ? "urls" : "torrents", !!textArea ? textArea : file!),
	...
	const [filterSearch, setFilterSearch] = useLocalStorage("home-filter-search", "");
	const [filterCategory, setFilterCategory] = useLocalStorage("home-filter-category", "Show All");
	const [filterStatus, setFilterStatus] = useLocalStorage("home-filter-status", "Show All");
	...
	const Torrents = useMemo(() => {
		...
		return Object.entries(torrentsTx)
			...
			?.filter(([hash, torr]) => (filterCategory !== "Show All" ? torr.category === filterCategory : true))
			?.filter(([hash, torr]) => (filterStatus !== "Show All" ? torr.state === filterStatus : true))
							...
							rowRenderer={({
								...
								<div key={key}>
									<TorrentBox
										...
										style={{
											...style,
											paddingBottom: index === Torrents.length - 1 ? "30vh" : undefined,
										}}
									/>
								</div>
							)}
