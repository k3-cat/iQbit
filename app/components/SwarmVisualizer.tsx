...
function SwarmDotWithMemoPosition(props: {
	...
	return (
		<Box
			...
			maxW={10}
			maxH={10}
			bgColor={props.strings.includes(`row-${props.row}`) && props.strings.includes(`col-${props.col}`) ? "blue.500" : "gray.500"}
		/>
	);
}

const SwarmVisualizer = ({ connected, label, height, swarm }: SwarmVisualizerProps) => {
	const rowAmount = 5;
	const amountPerRow = 20;

	const connectedIndex = useMemo(() => {
		return Array.from(Array(connected > 0 ? Math.floor((connected / swarm) * 100) || 1 : 0).keys())
			.map(() => {
