							...
							<FormLabel>
								...
								<Switch
									...
									onChange={(event) => {
										...
										updateSetting(props?.settingKey, event.target.checked ? props.disableDefaultValue || "" : -1);
									}}
								/>
							</FormLabel>
						</FormControl>
				...
				<FormControl>
					...
					<InputGroup size="lg">
						<Input
							...
							value={props.withToggle && !enabled ? props.disableDefaultValue || "" : settings?.[props?.settingKey]?.toString() || ""}
							onChange={(e) =>
								updateSetting(
									props.settingKey,
									typeof settings?.[props.settingKey] === "number" ? parseInt(e.target.value) : e.target.value,
								)
							}
						/>
