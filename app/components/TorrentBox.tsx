...
const TorrentBox = ({ torrentData, hash, categories, loading, style }: TorrentBoxProps) => {
	...
	const [waiting, setWaiting] = useState<"" | "mainBtn" | "category" | "name" | "sequential" | "firstLastPriority" | "autoManagement">();

	useEffect(() => {
		setWaiting("");
	}, [torrentData.state, torrentData.category, torrentData.name, torrentData.seq_dl, torrentData.f_l_piece_prio, torrentData.auto_tmm]);

	const { mutate: pause } = useMutation("pauseTorrent", () => TorrClient.pause(hash), {
		onMutate: () => setWaiting("mainBtn"),
		onError: () => setWaiting(""),
	});

	const { mutate: resume } = useMutation("resumeTorrent", () => TorrClient.resume(hash), {
		onMutate: () => setWaiting("mainBtn"),
		onError: () => setWaiting(""),
	});

	const deleteConfirmationDisclosure = useDisclosure();
	const { mutate: remove } = useMutation("deleteTorrent", (deleteFiles: boolean) => TorrClient.remove(hash, deleteFiles), {
		onMutate: () => setWaiting("mainBtn"),
		onError: () => setWaiting(""),
	});

	const categoryChangeDisclosure = useDisclosure();
	const { mutate: changeCategory } = useMutation("changeCategory", (category: string) => TorrClient.setTorrentCategory(hash, category), {
	...
	const { mutate: renameTorrent, isLoading: renameLoading } = useMutation("changeCategory", () => TorrClient.renameTorrent(hash, newName), {
		onMutate: () => setWaiting("name"),
		onError: () => setWaiting(""),
		onSuccess: () => renameTorrentDisclosure.onClose(),
	});

	const { mutate: toggleSequentialDownload } = useMutation("sequential-download", () => TorrClient.toggleSequentialDownload(hash), {
		onMutate: () => setWaiting("sequential"),
		onError: () => setWaiting(""),
	});

	const { mutate: toggleFirstLastPiecePrio } = useMutation("first-last-priority", () => TorrClient.toggleFirstLastPiecePrio(hash), {
	...
	const { mutate: toggleAutoManagement } = useMutation(
		"first-last-priority",
		async () => {
			return TorrClient.setAutoManagement(hash, (!torrentData.auto_tmm).toString());
		},
		{
	...
	return (
		<div style={style}>
					...
					{isPaused || (
						<Flex alignItems={"center"} gap={4} flexGrow={2}>
							<StatWithIcon
								...
								label={isDownloading ? torrentData.num_seeds : isDone ? torrentData.num_leechs : 0}
							/>
							<StatWithIcon
								lit={isDone ? torrentData.upspeed > 0 : torrentData.dlspeed > 0}
								icon={<IoSpeedometer />}
								label={
									(isPaused ? 0 : isDone ? filesize(torrentData.upspeed, { round: 1 }) : filesize(torrentData.dlspeed, { round: 1 })) + "/s"
								}
							/>
						</Flex>
					)}
			...
			<IosBottomSheet title={"Rename Torrent"} disclosure={renameTorrentDisclosure}>
				<VStack gap={10}>
					<FormControl>
			...
			<IosBottomSheet title={"Torrent Information"} disclosure={TorrentInformationDisclosure} modalProps={{ size: "3xl" }}>
