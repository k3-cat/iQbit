...
export function PosterGrid<T>(props: PosterGridProps<T>) {
	...
	return (
		<Grid gap={2} pt={2} width={"100%"} justifyContent={"flex-start"} templateColumns={"repeat( auto-fit, minmax(150px, 1fr) )"} mb={130}>
			{props.list.map((movie) => (
				<AspectRatio
					...
					overflow={"hidden"}
					style={{
						backgroundImage: `url(${props.images(movie).large}), url(${props.images(movie).small})`,
					}}
				>
