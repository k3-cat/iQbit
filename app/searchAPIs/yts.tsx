...
const YTSSearch = (props: SearchProviderComponentProps) => {
	...
	const { mutate, data, isLoading } = useMutation(
			...
			}),
		{
			...
			onSuccess: (data) => {
				...
				data?.movies.forEach((movie) => movie.torrents.forEach((torr) => sourceList.add(torr.type)));
	...
	const [expandedDescription, { toggle: toggleExpandedDescription, off: closeExpandedDescription }] = useBoolean(false);
		...
		return (data?.movies || [])
			.filter((movie) => {
				if (props.filterState.selectedSource !== "") {
					return movie.torrents.some((torr) => torr.type === props.filterState.selectedSource);
				} else {
					return true;
				}
			})
			.filter((movie) => {
				if (props.filterState.minSeeds !== "0") {
					return movie.torrents.some((torr) => torr.seeds >= parseInt(props.filterState.minSeeds || "0"));
				} else {
					return true;
				}
			});
	}, [data?.movies, props.filterState.selectedSource, props.filterState.minSeeds]);
	...
	return (
		<VStack>
			...
			<PosterGrid
				list={filteredMovies}
				keyExtractor={(movie) => movie.id + movie.date_uploaded_unix?.toString()}
				images={(movie) => ({
							...
							return (
								<TorrentDownloadBox
									...
									magnetURL={createYTSMagnetLink(
										torrent.hash,
										`${selectedMovie?.title} (${selectedMovie?.year || "--"})` || "Title not found",
									)}
