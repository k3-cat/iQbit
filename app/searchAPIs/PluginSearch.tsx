...
const PluginSearch = (props: SearchProviderComponentProps) => {
	...
	const stickyBgColorInTab = useColorModeValue(isLarge ? "whiteAlpha.800" : "whiteAlpha.300", "blackAlpha.500");
	...
	const { mutate: deleteSearch } = useMutation("deleteSearch", () => TorrClient.deleteSearch(searchId!), {
		onSuccess: () => setSearchId(undefined),
	});

	const { mutate: stopSearch } = useMutation("stopSearch", () => TorrClient.stopSearch(searchId!), {
		onSuccess: () => deleteSearch(),
	});

	const { data } = useQuery("getSearches", () => TorrClient.getResults(searchId!), {
	...
	}, [data, props.filterState.selectedSource, props.filterState.minSeeds, props.filterState.qualitySelected]);
	...
	return (
		<VStack>
			...
			{filteredResults.map((result) => (
				<TorrentDownloadBox key={result.fileUrl} magnetURL={result.fileUrl} title={result.fileName}>
					<SeedsAndPeers seeds={result.nbSeeders.toString()} peers={result.nbLeechers.toString()} size={result.fileSize} />
				</TorrentDownloadBox>
			))}
		</VStack>
	);
