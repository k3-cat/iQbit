...
const RarbgSearch = (props: SearchProviderComponentProps) => {
	const {
		...
		data,
		isLoading,
	} = useMutation("rarbgSearch", () => rarbgAPI.search(props.searchState[0], props.category as keyof typeof RarbgCategoryDictionary), {
		onMutate: () =>
			ReactGA.event({
				action: "executed",
				category: "search",
	...
	return (
		<VStack>
			...
			<Flex flexDirection="column" gap={2} width="100%">
				{(!data?.torrent_results?.length || true) && <Filters {...props.filterState} />}
				{data && (
					<SectionSM title="Results" titleRight={<CategorySelect category={addToCategory} onSelected={setAddToCategory} />}>
						{filteredMovies?.map((torr) => (
							<TorrentDownloadBox key={torr.download} title={torr.title} magnetURL={torr.download} category={addToCategory}>
								...
								<SeedsAndPeers seeds={torr.seeders.toString()} peers={torr.leechers.toString()} />
							</TorrentDownloadBox>
						))}
					</SectionSM>
				)}
