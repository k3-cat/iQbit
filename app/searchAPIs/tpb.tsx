...
const ApiDomain = process.env.NODE_ENV === "development" && useLocalServer ? devURL : "https://iqbit.app/";
...
const TPBSearch = (props: SearchProviderComponentProps) => {
	const {
		...
		"tpbSearch",
		async () => {
			const { data } = await axios.get<TPBRecord[]>(`${ApiDomain}api/tpb/search`, {
				params: {
		...
		{
			onMutate: () => ReactGA.event({ action: "executed", category: "search", label: "TPB" }),
			onSuccess: (data) => {
		...
		return (data || [])
			...
			.filter((Torr) => {
				if (props.filterState.minSeeds !== "0") {
					return parseInt(Torr.seeders) >= parseInt(props.filterState.minSeeds || "0");
				} else {
					return true;
				}
			});
